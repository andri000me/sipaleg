<?php
  ...
  $jumlahUser = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tb_user_login"));

  $users = mysqli_query($conn, "SELECT * FROM tb_user_login");
  $dataUsers = getData($users);
  $dataPerPage = 5;
  if (isset($_GET)){
    if (empty($_GET['page'])) {
      $page = 1;
  ...
  $jumlahPage = ceil($jumlahUser/$dataPerPage);
  $akhirPage = $page*$dataPerPage;
  if ($akhirPage == ($jumlahPage*$dataPerPage) ) {
    $akhirPage = $page*$dataPerPage-($page*$dataPerPage-$jumlahUser);
  }
?>
<!DOCTYPE HTML>
<html>
      ...
      <!-- /.navbar-static-side -->
	   ...
	   <div class="xs">
	     <div class="bs-example4" data-example-id="contextual-table">
            ...
            <a href="#" class="btn btn-primary"><i class="material-icons">&#xE24D;</i> <span>Export to Excel</span></a>
	         ...
	         <table class="table table-striped table-hover">
	           ...
	           <tbody>
	             ...
	             <tr>
                 <?php $user = $dataUsers[$i]; ?>
	               <td><?= $i+1 ?></td>
                  <td><?= $user['username_login'] ?></td>
	               <td><?= $user['password'] ?></td>
	               <td><?= $user['email'] ?></td>
	               <td>
	                 <a href="edit_data?user=<?= $user['id'] ?>" class="settings" title="Edit"><i
                  class="material-icons">&#xE8B8;</i></a>
	                 <a href="delete_data?user=<?= $user['id'] ?>" class="delete" title="Delete" data-toggle="tooltip">
           ...
           <div class="clearfix">
              <div class="hint-text">Showing <b><?= $akhirPage-$dataPerPage*($page-1) ?></b> out of <b><?= $jumlahUser ?></b> entries</div>
              <ul class="pagination">
                <?php if ($page != 1) : ?>
