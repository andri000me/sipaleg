<?php
  ...
  $jumlahPartai = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tb_user_login"));

  $partais = mysqli_query($conn, "SELECT * FROM tb_partai");
  $dataPartais = getData($partais);
  $dataPerPage = 5;
  if (isset($_GET)){
    if (empty($_GET['page'])) {
      $page = 1;
  ...
  $jumlahPage = ceil($jumlahPartai/$dataPerPage);
  $akhirPage = $page*$dataPerPage;
  if ($akhirPage == ($jumlahPage*$dataPerPage) ) {
    $akhirPage = $page*$dataPerPage-($page*$dataPerPage-$jumlahPartai);
  }
?>
<!DOCTYPE HTML>
<html>
<head>
<title>Partai Management | SIPALEG ADMIN</title>
      ...
      <!-- /.navbar-static-side -->
	   ...
	   <div class="xs">
	     <div class="bs-example4" data-example-id="contextual-table">
            ...
            <a href="#" class="btn btn-primary"><i class="material-icons">&#xE24D;</i> <span>Export to Excel</span></a>
	         ...
	         <table class="table table-striped table-hover">
	           ...
	           <tbody>
	             ...
	             <tr>
                 <?php $partai = $dataPartais[$i]; ?>
	               <td><?= $i+1 ?></td>
                  <td><img style="max-height: 50px;" src="../assets/img/partai/<?= $partai['foto_partai'] ?>" class="avatar"
	                   alt="Avatar+<?= $i ?>"></td>
	               <td><?= $partai['id'] ?></td>
	               <td><?= $partai['nama_partai'] ?></td>
	               <td>
	                 <a href="edit_data?partai=<?= $partai['id'] ?>" class="settings" title="Edit"><i
                  class="material-icons">&#xE8B8;</i></a>
	                 <a href="delete_data?partai=<?= $partai['id'] ?>" class="delete" title="Delete" data-toggle="tooltip">
           ...
           <div class="clearfix">
              <div class="hint-text">Showing <b><?= $akhirPage-$dataPerPage*($page-1) ?></b> out of <b><?= $jumlahPartai ?></b> entries</div>
              <ul class="pagination">
                <?php if ($page != 1) : ?>
